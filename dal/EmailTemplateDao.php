...
abstract class EmailTemplateDao {
    public static function cadastrar(EmailTemplate $t): int {
        try {
            $pdo = Conn::getConn();
            $sql = $pdo->prepare("INSERT INTO email_templates (titulo, assunto, corpo) VALUES (:titulo, :assunto, :corpo)");
    ...
    public static function listar(): array {
        ...
        return array_map(fn($d) => EmailTemplate::criar(
            (int)$d["id"], $d["titulo"], $d["assunto"], $d["corpo"]
        ), $res);
    }
    ...
    public static function buscarPorId(int $id): ?EmailTemplate {
        ...
        if (!$d) return null;
        return EmailTemplate::criar((int)$d["id"], $d["titulo"], $d["assunto"], $d["corpo"]);
    }

    public static function editar(EmailTemplate $t): void {
        $pdo = Conn::getConn();
        $sql = $pdo->prepare("UPDATE email_templates SET titulo=?, assunto=?, corpo=? WHERE id=?");
        $sql->execute([$t->getTitulo(), $t->getAssunto(), $t->getCorpo(), $t->getId()]);
        if ($sql->rowCount() !== 1) {
